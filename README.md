...
基于 Next.js 16 + React 19 + Vercel AI SDK 构建的本地 AI 对话工作平台，支持多会话管理、文件 RAG、语义搜索、上下文压缩和**开发问题文档系统**。
...
4. 点击"保存文档"
5. 在 [/docs](http://localhost:3000/docs) 页面查看所有文档
...
2. **保存触发**：点击"保存为文档"按钮
3. **AI 提取**：调用 GPT-4o-mini 自动提取结构化信息
   - 标题：问题的简短概括
   - 分类：技术栈分类（React/Next.js/TypeScript 等）
   - 标签：3-5 个关键词
   - 问题描述：详细的问题说明
   - 解决方案：具体的解决步骤
   - 代码示例：相关代码片段
...
- **运行环境**：浏览器端通过 WebAssembly 运行
- **性能优化**：模型单例复用，首次加载 ~25MB，后续缓存复用
...
- **sessions 表**：会话元数据（标题、创建时间、置顶状态、摘要）
...
### localStorage（兼容旧版本）
- **ai-sessions**：旧版会话数据（可通过迁移工具导入 SQLite）
...
### 进行中 🚧
- [ ] 完整的 SQLite 会话迁移（useSessionWithDB）
- [ ] 文档编辑功能
- [ ] 文档全文搜索（SQLite FTS5）
