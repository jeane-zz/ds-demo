...
- **本地工具调用**：模型可按需查询本地文档库、当前会话历史和上传文件片段

### 2. 本地工具调用（Tool Use）
- **文档检索工具**：查询已保存的开发问题文档、分类、标签和历史解决方案
- **会话检索工具**：查询当前会话中较早的讨论内容和结论
- **上传文件检索工具**：查询用户消息中上传过的文件片段
- **调用规则增强**：服务端在工具决策阶段注入专门的 tool-use 指令，减少误调和漏调
- **结构化结果**：工具返回统一的 `sourceType` 与 `*Snippet` 字段，便于模型总结引用
...
1. **工具决策**：服务端先让模型基于当前对话和 tool-use 指令判断是否需要调用工具
2. **按需检索**：模型可调用 `search_documents`、`search_conversation`、`search_uploaded_files`
3. **结构化返回**：工具结果以 JSON 返回，包含 `sourceType` 和片段字段（如 `contentSnippet`）
4. **多轮工具调用**：单次请求最多执行 2 轮工具调用，避免循环调用
5. **最终回答**：工具结果进入模型上下文，最终仍以普通文本流式返回给前端
...
- [x] 文件 RAG
- [x] 对话语义搜索
- [x] 本地工具调用（Tool Use：文档 / 会话 / 上传文件检索）
- [x] 上下文压缩
- [x] 虚拟化渲染
...
- [x] 本地模型集成（Ollama 本地优先 + DeepSeek fallback）
